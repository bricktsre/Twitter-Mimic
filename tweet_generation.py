...
class TweetGeneration(nn.Module):
  ...
  def forward(self, history, state=None):
    ...
    batch_size, history_steps,_ = data.shape
    state = None
    for char in range(history_steps-1):
      state,_ = self.rnn_model(data[:,char, :], state)
    
    state = self.rnn_model(data[:,history_steps-1, :],state)
    outputs = None
...
def tweet_trainer():
  train_dataset = TweetDataset(txt_path='trump.txt',
  ...
  model = TweetGeneration(vocabulary_size=len(vocabulary),
...
def tweet_writer():
  ...
  for s in range(FLAGS.generation_length):
    output,_ = model(inputs)
    next_char = sample_next_char_id(output)
    #print(next_char.item())
    #print(inputs.shape)
    inputs = torch.cat((inputs,next_char.view(1,1)),1)
    generated_chars.append(index2char[next_char.item()])
  #####################################################################
  f = open(start_string + ".txt","w+")
  f.write(start_string + ''.join(generated_chars))
  f.close()
...
def main(unused_argvs):
  if FLAGS.task_type == 'training':
    #twtRet = TweetRetriever("out.txt")
    #output = twtRet.getTweets(sys.argv[1])
    #print(output)
    tweet_trainer()
